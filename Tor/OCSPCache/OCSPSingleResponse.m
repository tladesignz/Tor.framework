...
+ (Error*)expiredWithResponse:(OCSP_SINGLERESP*)response expired:(BOOL*)expired {
    ...
    OCSP_single_get0_status(response, NULL, NULL, NULL, &nextupd);

    ret = ASN1_TIME_diff(&pday, &psec, NULL, nextupd);
    if (ret == 0) {
...
+ (Error*)datesFromResponse:(OCSP_SINGLERESP*)response
    ...
    OCSP_single_get0_status(response, NULL, NULL, &thisupd, &nextupd);

    unsigned char *t = ASN1_STRING_data(thisupd);
    unsigned char *n = ASN1_STRING_data(nextupd);
